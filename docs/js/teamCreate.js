...
/**
 * チームNoからチーム名を生成する関数
 * 1 → A, 2 → B, ... , 27 → AA, 28 → AB, ....infinity
 * @param {number} i チームナンバー
 * @returns {String} 生成したチーム名 
 */
const getTeamName = function(i) {
    ...
    while (n >= 1) {
        n--;
        str = String.fromCharCode(charCodeInit + (n % AlphaBetNum)) + str;
...
/**
 * チームの定員を取得して、チーム編成を行い、画面に出力する関数
 */
...
/**
 * 1チームの定員を取得して、チーム編成を行い、画面に出力する関数
 */
// const memberShuffle = () => {
//     let outputTxt = "";
//     let count = 0;
